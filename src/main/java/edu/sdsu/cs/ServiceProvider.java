...
class ServiceProvider
{
    ...
    protected static void WriteToFile(Statistics statistics) throws IOException {
        ...
        String data = String.format("%20s:%d" , "Length of longest line in file",
                statistics.getLengthOfLongestLine())
                    .concat(String.format("%20s:%.2f" , "\nAverage line length",
                            statistics.getAverageLineLength()))
                    .concat(String.format("%20s:%d" , "\nNumber of unique space-delineated tokens (case-sensitive)",
                            statistics.getSensitiveNumberOfSpaceDelimitedTokens()))
                    .concat(String.format("%20s:%d" , "\nNumber of unique space-delineated tokens (case-insensitive)",
                            statistics.getInsensitiveNumberOfSpaceDelimitedTokens()))
                    .concat(String.format("%20s:%d" , "\nNumber of all space-delineated tokens in file",
                            statistics.getNumberOfAllSpaceDelimitedTokens()))
                    .concat(String.format("%20s:%s" , "\nMost frequently occurring token(s)",
                            statistics.getMostFrequentlyOccuringToken()))
                    .concat(String.format("%20s:%d" , "\nCount of most frequently occurring token (case-insensitive)",
                            statistics.getInsensitiveCountOfMostFrequentlyOccuringToken()))
                    .concat(String.format("%20s:%s" , "\n10 most frequent tokens with their counts (case-insensitive)",
                            GetFormatedString(statistics.getInsensitiveTenMostFrequentlyUsedTokens())))
                    .concat(String.format("%20s:%s" , "\n10 least frequent tokens with their counts (case-sensitive)",
                            GetFormatedString(statistics.getInsensitiveTenLeastFrequentlyUsedTokens())));
